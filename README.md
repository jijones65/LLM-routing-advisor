...
- **Complete project-document import** — upload any readable project document as PDF or DOCX; the importer preserves the extracted outline, paragraphs, lists, code and table rows in an ordered Markdown appendix while a separate bounded evidence set drives editable suggestions. Visible coverage distinguishes source retention from inference, weak guesses remain for review, and existing team/model architecture stays separate from advisor candidates
...
- **Complete-team validation** — each saved plan turns its application requirements, Skills and assigned models into job-specific workflows, a 100-point rubric, detailed comparison rules, five trial guides, environment/Python runner commands and model-neutral trial-writing prompts; upload completed evidence to assess quality, total cost, P95 speed, safety, routing, provider failure and peak load
